...
;; interpreter does?
;;
;; `canonical_kotoba_test.clj` drives the guest through `kotoba.kir`, which is not what
...
(ns dnssec.canonical-artifact-test
  ...
  (delay
    (let [{:keys [kotoba nbb runtime]} @tools]
      (if-not (and kotoba nbb runtime)
        ...
        (let [r (shell/sh nbb "test/dnssec/artifact_probe.cljs" kotoba amu-root)
...
(def ^:private kir
  (delay (:kir (compiler/compile-project
                {'dnssec.canonical (slurp (io/file (System/getProperty "user.dir") "kotoba/dnssec/canonical.kotoba"))}
...
(deftest the-native-backend-admits-this-guest
  ...
  (if-let [bin (:kotoba @tools)]
    (let [out (str (System/getProperty "java.io.tmpdir") "/dnssec-canonical-native.kexe")
          r (shell/sh bin "-M" "compile"
                      (str (io/file (System/getProperty "user.dir") "kotoba/dnssec/canonical.kotoba"))
